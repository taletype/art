...
import type { AuctionSummary } from "@/types/auction-v1";
    ...
    <article className="overflow-hidden rounded-[1.6rem] border border-white/10 bg-white/[0.03]">
      ...
      <div className="space-y-5 p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <span className="status-pill shrink-0">{formatCountdown(auction.endAt)}</span>
        </div>
        ...
        <dl className="grid grid-cols-2 gap-3 text-sm">
          <div className="rounded-2xl border border-white/8 bg-black/20 p-3">
            ...
            <dd className="mt-1 font-semibold text-white">{auction.startPrice.toFixed(2)} SOL</dd>
          </div>
          <div className="rounded-2xl border border-white/8 bg-black/20 p-3">
            <dt className="text-white/45">High bid</dt>
            <dd className="mt-1 font-semibold text-white">
              {auction.highestBid ? `${auction.highestBid.toFixed(2)} SOL` : "No bids"}
            </dd>
          </div>
          <div className="rounded-2xl border border-white/8 bg-black/20 p-3">
            <dt className="text-white/45">Min increment</dt>
            <dd className="mt-1 font-semibold text-white">{auction.minIncrement.toFixed(2)} SOL</dd>
          </div>
          <div className="rounded-2xl border border-white/8 bg-black/20 p-3">
